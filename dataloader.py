...
class Data:
    ...
    def __init__(self, args):
        set_seed(args.seed)
        max_seq_lengths = {'clinc':30,'clinc_aug':30, 'stackoverflow':45,'banking':55,'hwu64':10,'snips': 35}
        ...
        #self.known_label_list =self.all_label_list[0:14] 
        ...
        """
        self.known_label_list=list(np.column_stack(self.known_label_list,np.ones(len(self.known_label_list))))
        self.unknown_label_list=list(np.column_stack(self.unknown_label_list,np.ones(len(self.unknown_label_list))))
        self.all_label_list=self.known_label_list+self.unknown_label_list
        """
        #self.deptree = spacy.load('en_core_web_sm')
        ...
        self.test_labeled_dataloader = self.get_loader1(self.test_labeled_examples,self.known_label_list, args, "test")
        self.test_unlabeled_dataloader = self.get_loader1(self.test_unlabeled_examples,self.unknown_label_list, args, "test")
        """
        ...
        self.train_labeled_examples_list=self.train_labeled_examples
        ...
        center_train_examples=len(self.train_labeled_examples)//2
        self.test_examples,self.test_IND_examples,self.test_OOD_examples = self.get_examples(processor, args, 'test')
        
        self.train_labeled_dataloader = self.get_loader(self.train_labeled_examples, args, 'pretrain')
        self.train_unlabeled_dataloader = self.get_loader(self.train_unlabeled_examples, args, 'train')
        self.train_dataloader=self.get_train_loader(self.train_labeled_examples,self.train_unlabeled_examples, args, 'train')
        ...
        self.semi_input_ids, self.semi_input_mask, self.semi_segment_ids, self.semi_label_ids = self.get_unlabeled(self.train_labeled_examples, self.train_unlabeled_examples, args)
        self.train_unlabeled_dataloader = self.get_semi_loader(self.semi_input_ids, self.semi_input_mask, self.semi_segment_ids, self.semi_label_ids, args)

        self.eval_dataloader = self.get_loader(self.eval_examples, args, 'eval')
        self.test_dataloader,self.test_IND_dataloader,self.test_OOD_dataloader = self.get_loader(self.test_examples, args, 'test'),self.get_loader(self.test_IND_examples, args, 'test_IND'),self.get_loader(self.test_OOD_examples, args, 'test_OOD') 
        self.n_known_cls=int(len(self.all_label_list))
    ...
    def get_examples(self, processor, args, mode = 'train'):
        ...
        if mode == 'train':
            ...
            test_IND_examples,test_OOD_examples = [],[]
            for example in ori_examples:
                if example.label in self.known_label_list:
                    test_IND_examples.append(example)
                else:
                    test_OOD_examples.append(example)
            return ori_examples,test_IND_examples,test_OOD_examples
    ...
    def get_unlabeled(self, labeled_examples, unlabeled_examples, args):
    ...
    def get_loader(self, examples, args, mode = 'train'):
        ...
        elif mode == 'eval' or mode == 'test' or mode == 'test_IND'or mode == 'test_OOD':
            sampler = SequentialSampler(data)
            dataloader = DataLoader(data, sampler=sampler, batch_size = args.eval_batch_size) 
        
        return dataloader
    #1
    def get_train_loader(self, examples_known,examples_unknown, args, mode = 'train'):
        tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=True)    
        
        features_known = convert_examples_to_features(examples_known, self.all_label_list, args.max_seq_length, tokenizer)

        input_ids = torch.tensor([f.input_ids for f in features_known], dtype=torch.long)
        input_mask = torch.tensor([f.input_mask for f in features_known], dtype=torch.long)
        segment_ids = torch.tensor([f.segment_ids for f in features_known], dtype=torch.long)
        label_ids = torch.tensor([f.label_id for f in features_known], dtype=torch.long)
        if_known=torch.tensor([1 for f in features_known], dtype=torch.long)
        data_known = TensorDataset(input_ids, input_mask, segment_ids, label_ids,if_known)

        features_unknown = convert_examples_to_features(examples_unknown, self.all_label_list, args.max_seq_length, tokenizer)
        input_ids = torch.tensor([f.input_ids for f in features_unknown], dtype=torch.long)
        input_mask = torch.tensor([f.input_mask for f in features_unknown], dtype=torch.long)
        segment_ids = torch.tensor([f.segment_ids for f in features_unknown], dtype=torch.long)
        label_ids = torch.tensor([0 for f in features_unknown], dtype=torch.long)
        if_known=torch.tensor([0 for f in features_unknown], dtype=torch.long)
        data_unknown = TensorDataset(input_ids, input_mask, segment_ids, label_ids,if_known)

        data=data_known+data_unknown
    ...
    def get_loader1(self, labelled_examples, label_list, args, mode="train"):
        tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=True)
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        text_tree=example.text_a
        for token in doc:
            #print(token.dep_)
            if token.dep_ == 'ROOT':
                text_tree=text_tree+token.text
            if token.dep_ == 'nsubj':
                text_tree=text_tree+token.text
            if token.dep_ == 'dobj':
                text_tree=text_tree+token.text
